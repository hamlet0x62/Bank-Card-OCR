...
from detection.utils.dataset import data_provider as data_provider
...
tf.app.flags.DEFINE_string('logs_path', '../log-dir/', '')
...
def main(argv=None):
    ...
    input_image = tf.placeholder(tf.float32, shape=[1, None, None, 3], name='input_image')
    ...
    with tf.Session(config=None) as sess:
        if FLAGS.restore:
            ...
            print(ckpt)
            restore_step = int(ckpt.rsplit('.')[-2].split('_')[-1])
